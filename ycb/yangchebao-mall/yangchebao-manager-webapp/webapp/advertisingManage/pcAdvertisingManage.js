$(document).ready(function(){ 
    var wraper = $('#Wraper_pcAds_manage');
	...
	fields_params['pcAdPicFile'] = {
	   ...
	   beforeSubmit: function(obj){
		  ...
		  if(re.test(obj.val())){
			...
			$("div[name='pcAdPicFile']",_form).find("input[type='file']").val('');
			return false;
		  }
		},
		single: true,
	...
	page_column_model.push({display: 'banner图', name : 'pcAdPicSmallAddress',width:80,diy:function($row,$div){
			var imgUrl = $row.pcAdPicSmallAddress;
    ...
    $('#Form_pcAds_manage .jrad-btn-normal',wraper).button({
		click: function(){
	...
	$('#Form_pcAds_manage .jrad-btn-primary',wraper).button({
		click: function(){
			var flag = $('#Form_pcAds_manage',wraper).form('validateAll');
			var json = $('#Form_pcAds_manage',wraper).form('getValue'); 
			...
			$.jRadPost({
				    	...
				    	success:function(){
							...
							$('.jrad-table',wraper).slideDown();  
							$(".scroll-up-btn").click();
							$('#Table_pcAds_manage').flexMessage('创建成功', 'success');
							$('#Table_pcAds_manage').flexReload();
				    	},error:function(data){
			...
			$.jRadPost({
				    	...
				    	success:function(){
							...
							$('.jrad-table',wraper).slideDown();  
							$(".scroll-up-btn").click();
							$('#Table_pcAds_manage').flexMessage('修改成功', 'success');
							$('#Table_pcAds_manage').flexReload();
				    	},error:function(data){
				    		var mes = eval('('+data.responseText+')');
				    			$.jRadMessage({
...
//	}); 
	...
	page_list_buttons.push({title: '创建',name:'Add', bclass: 'add',onpress : function(){
			 initAdsInfo(wraper); 
			 $('#Form_pcAds_manage',wraper).form({item:{'adTurnType':'0'}}); 
	...
	page_list_buttons.push({title: '修改',name:'Edit', bclass: 'edit',prefunc:function(){
            var checked = $('#Table_pcAds_manage').getCheckedTrs();
	...
	$('#Table_pcAds_manage').flexigrid({
			...
			showSearch:true, 
			url:'/ycbmall-manager-ws/ws/0.1/AdvertisingCms/queryPcAdvertisinge',
	...
	$(".searchButtons .ui-btn-icon:first",wraper).click(function(){
			 $('#Table_pcAds_manage',wraper).flexOptions({url:'/ycbmall-manager-ws/ws/0.1/AdvertisingCms/queryPcAdvertisinge'});
			 $('#Table_pcAds_manage',wraper).flexReloadSearch();
	}); 
	...
	$(".searchButtons .ui-btn-icon:nth-child(2)",wraper).click(function(){  
			...
			$('#Table_pcAds_manage').flexOptions({
						url:'/ycbmall-manager-ws/ws/0.1/AdvertisingCms/queryPcAdvertisinge',
						queryParam:{},
						page:0});
			$('#Table_pcAds_manage').flexReloadSearch();
	}); 
...
function initAdsInfo(wraper){
		//前台排序为0就只读状态
		$('#Form_pcAds_manage div[name="status"]').select('readonly',false);
	   //广告图
	   var _ul = $("div[name='pcAdPicFile']",wraper).children(".pic-show"); 
	   _ul.html(""); 
	  //地区
	   $("#Form_pcAds_manage div[name='regions']",wraper).data("areaList",[]); 
	   $("#Form_pcAds_manage #area_check_string",wraper).html("");   
}
//修改时赋值
function Form_pcAds_manageVal(wraper,item){ 
//   $("#serviceNames",wraper).html(item.serviceName);
   var _ul = $("div[name='pcAdPicFile']",wraper).children(".pic-show"); 
   var src = item.adPicSmallAddress;
   var srcBig = item.adPicAddress;
   if(src!=undefined&&src!=""){ 
	  var _li = '<li name="" class="pcAdsPicBoxli"><div class="fileUrl-pic"><div class="pic-box"><div class="pic-content"><div class="pic-vc">'
				+'<a class="pcAdsFile cboxElement"  title="" href="'+srcBig+'" ><img src="'+src+'"></a>'
				+'</div></div><span name="small" class="del-btn"></span></div></div></li>'
	  _ul.html(_li); 
	  $(".pcAdsFile",wraper).colorbox({rel: 'file',close:'x',maxWidth: '70%',maxHeight: '70%',previous:'<i class="icon-font-arrow-left"></i>',next:'<i class="icon-font-arrow-right"></i>'});
	  $(".pcAdsPicBoxli .del-btn",wraper).click(function(){
			var small = $(this).prev(".pic-content").find("img").attr("src");
			$(this).parents(".pcAdsPicBoxli").remove(); 
   ...
   $("#Form_pcAds_manage div[name='regions']",wraper).data('areaList',areaList); 
...
function updateAdsView(id,wraper){ 
	 ...
	 $('.details-box',wraper).slideDown(); 
	 $(".scroll-up-btn").click();  
	 $('#Form_pcAds_manage',wraper).form({item: item});  
	 if(item.sequence==0){
		 $('#Form_pcAds_manage div[name="status"]').select('readonly',true);
	 }
	 item.id = id;
	 item.osInfo = item.osInfo.split(",");
	 item.location = item.location;
	 $('#Form_pcAds_manage',wraper).form({item: item});  
	 Form_pcAds_manageVal(wraper,item); 
} 
