...
var index = {
    ...
    newSubmit : function() {
        ...
        if (userid == "" || password == ""||captcha=="") {
            if (userid == "") {
                $('#emailerror').html("<span>请输入登录邮箱</span>");
        ...
        $.ajax({
            url : "/mall-euc-ws/ws/0.1/login/withcaptcha?username=" + userid + "&password=" +hex_md5( password) + "&domain=10&captcha=" + captcha,
            ...
            error : function (xhr) {
				...
				if (errormsg != undefined) { 
					$('#captchaerror').html("<span>"+errormsg[0].message+"</span>"); 
					$('#captchaerror').css("display", "block"); 
				}else{
					$('#captchaerror').html("<span>用户名/邮箱或密码错误</span>"); 
					$('#captchaerror').css("display", "block");
				}
            }
    });
    ...
    refreshCaptchaImg : function(imageId) {
        $('#' + imageId)
                .attr("src", "/mall-euc-ws/ws/0.1/captcha?a=" + index.getRandomStr());
    },
    /**
     * 随机数字
     */
