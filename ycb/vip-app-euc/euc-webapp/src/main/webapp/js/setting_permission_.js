...
function showPermissionsTree(data) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		for (var j = 0; j < second; j++) {
			...
			d.add(first+j+1, i+1, '', '', data[i].moduleList[j].moduleName);
			var three = data[i].moduleList[j].operationList.length;
			for(var k = 0; k < three; k++) {
				d.add(data[i].moduleList[j].operationList[k].operationId, first+j+1, 'operation', '', data[i].moduleList[j].operationList[k].operationName);
			}
		}
	}
