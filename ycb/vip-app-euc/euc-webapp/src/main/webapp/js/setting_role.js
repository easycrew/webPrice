...
function queryPageControlCallBack(page_index, container) {
    if (!initPagination) {
        queryRoleByName(page_index, 10, lastQueryParam);
    }
}

var lastQueryParam;

function queryRoleByName(pageindex, pagesize, queryParam) {
    lastQueryParam = queryParam;
    var url = "/euc/ws/0.1/role/query?name=" + encodeURIComponent(queryParam) + "&pageindex=" + pageindex + "&pagesize=" + pagesize;
    var success = function(data) {
        if (initPagination) {
            $("#paging_panel").pagination(data.totalPages * pagesize, {
                num_edge_entries : 1,
        ...
        showRoleList(data);
        if (data.items.length == 0) {
            showErrorDialog("info", "没有找到对应权限组");
        }
    };
