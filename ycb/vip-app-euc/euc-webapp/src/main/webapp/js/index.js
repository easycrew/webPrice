...
var index = {
    ...
    newSubmit : function() {
        ...
        if (userid == "" || password == "") {
            if (userid == "") {
                $('#emailerror').html("<font>请输入登录邮箱</font>");
        ...
        $.ajax({
        url : "/euc/ws/0.1/login?username=" + userid + "&password=" +hex_md5( password) + "&domain=" + $('#domain').val(),
        ...
        error : function (xhr) {
            $('#errorInfo').html("<font>邮箱或密码错误</font>");
    ...
    refreshCaptchaImg : function(imageId) {
        $('#' + imageId)
                .attr("src", "/ws/validatecode/picture");
        $('#' + imageId)
                .attr("src", "/ws/validatecode/picture?a=" + index.getRandomStr());
    },
    /**
     * 随机数字
     */
    ...
    checkUserLoginOneFocus:function(id)
    {
       ...
       // if(result==0){
          ...
          if (account_no == "" || account_no == null) {
              ...
              $('#hiddendotype').val("no");
              $('#emailerror').html("<font>邮箱长度过长。</font>");
              $('#emailerror').attr("class","onError");
