...
$(document).ready(function() {
    ...
    $("#query").click(function() {
        ...
        if (userName != "" || departmentId != '-1' || positionId != '-1') {
            initPagination = true;
            queryUser(0, 10, userName, departmentId, positionId);
        } else {
...
function queryPageControlCallBack(page_index, container) {
    if (!initPagination) {
        queryUser(page_index, 10, lastUserName, lastDepartmentId, positionId);
    }
}
...
function queryUser(pageindex, pagesize, userName, departmentId, positionId) {
    ...
    var url = "/euc/ws/0.1/user/query?" + queryParam + "pageindex=" + pageindex + "&pagesize=" + pagesize;
    var success = function(data) {
        if (initPagination) {
            $("#paging_panel").pagination(data.totalPages * pagesize, {
                num_edge_entries : 1,
