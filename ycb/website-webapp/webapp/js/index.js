...
var index = {
	...
	newSubmit: function() {
		...
		if (userid == "" || password == ""||captcha=="") {
			if (userid == "") {
				$('#emailerror').html("<span>请输入用户名/手机号码</span>").show();
			}
		...
		$.ajax({
			url: "/vip-ws/ws/0.1/shopAccountBack/checkAccountStatus?username="+userid,
			...
			if (data.status=="1"){
			$.ajax({
			url : "/vip-euc-ws/ws/0.1/login/withcaptcha?username=" + userid + "&password=" +hex_md5( password) + "&domain=10&captcha=" + captcha,
			contentType: "application/json;charset=utf-8",
			dataType: "json",
			type: "POST",
			success: function(data) {
			...
			error: function(xhr) {
				...
				$('#errorInfo').html("<font>用户名/手机号码或密码错误</font>");
				$('#errorInfo').attr("class", "on-error");
    ...
    refreshCaptchaImg : function(imageId) {
        $('#' + imageId)
                .attr("src", "/vip-euc-ws/ws/0.1/captcha?a=" + index.getRandomStr());
    }, 
    refreshCaptchaImgcount : function(imageId) {
        $('#' + imageId)
                .attr("src", "/website-ws/ws/0.1/verify/genCode?a=" + index.getRandomStr());
    },
    /**
     * 随机数字
     */
    ...
    checkUserLoginOneFocus:function(id)
    {
       ...
       // if(result==0){
          ...
          if (account_no == "" || account_no == null) {
              ...
              $('#emailerror').html("<span>请输入用户名或手机号码。</span>");
...
/*          else if (/^([a-zA-Z0-9]+[_|\_|\.|\-]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/.test(account_no) == false) {
          ...
          if(account_no.length > 255){
              $('#hiddendotype').val("no");
              $('#emailerror').attr("style","display:block");
              $('#emailerror').html("<span>手机号码或用户名长度过长。</span>");
