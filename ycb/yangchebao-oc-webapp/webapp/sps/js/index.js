...
var index = {
    ...
    newSubmit : function() {
        ...
        var abbrev = $("#organizationAbbrev").val();
        var productCode = $("#productCode").val();
        var captcha = $("#captcha").val();
        if (account == "" || password == "") {
            if (account == "") {
                $('#accounterror').html("<font>请输入登录账号</font>");
                $('#accounterror').attr("class","onError");
            }

            if (password == "") {
                $('#passworderror').html("<font>请输入密码</font>");
        ...
        if (abbrev == "") {
            $('#errorInfo').html("<font>访问地址不正确</font>");
            $('#errorInfo').attr("class", "onError");
            $('#errorInfo').css("display", "block");
            return;
        }

        if (captcha == "") {
            $('#captchaerror').html("<font>验证码不能为空</font>");
            $('#captchaerror').attr("class","onError");
            return;
        }
        ...
        success : function(data) {
            ...
            if (strHref.indexOf("backurl=") != -1){
                ...
                strGoToUrl = window.location.pathname.split("/login")[0];
                if(window.location.pathname.indexOf('chechang_login')!=-1){
                    strGoToUrl = window.location.pathname.split("/chechang_login")[0];
                }
        ...
        error : function (xhr) {
            ...
            if (responseText.length > 0 && responseText[0].code == 'captcha') {
                $('#errorInfo').html("<font>验证码不正确,点击验证码可更换</font>");
            } else {
                $('#errorInfo').html("<font>用户名或密码错误</font>");
            }
     ...
     */
     logOut : function() {
        $.ajax({
            url : "/partner/ws/0.2/user/logout",
            contentType : "application/json;charset=utf-8",
            dataType : "json",
            type : "GET",
            success : function(data) {
    ...
    initLoginValidator : function() {
        ...
        $('#protocol').formValidator( {
            tipid : 'protocol_tip',
            onshow : '&nbsp;',
            onfocus : '<font>请阅读《车友互联网络服务使用协议》</font>',
            oncorrect : '&nbsp;'
        }).functionValidator( {
            ...
            onerror : '<font>请阅读《车友互联网络服务使用协议》</font>'
        });
        $('#account_no')
                .formValidator(
                        {
                            ...
                            onshow : '&nbsp;',
                            onfocus : '<font>请输入常用邮箱，如：xx@163.com，它将成为您未来的登录账号！</font>',
                            oncorrect : '&nbsp;'
                        })
                .regexValidator(
                        {
                    ...
                    onerror : '<font>您注册的账户已被使用，请重新输入</font>',
                    onwait : '<font>正在等待服务器验证结果...</font>'
                });
        $('#password').formValidator( {
            tipid : 'password_tip',
            onshow : '&nbsp;',
            onfocus : '<font>6～16个字符（字母、数字、特殊符号），区分大小写</font>',
            oncorrect : '&nbsp;'
        }).functionValidator( {
            ...
            onerror : '<font>6～16个字符（字母、数字、特殊符号），区分大小写</font>'
        });
        $('#rep_password').formValidator( {
            tipid : 'rep_password_tip',
            onshow : '&nbsp;',
            onfocus : '<font>请输入确认密码</font>',
            oncorrect : '&nbsp;'
        }).functionValidator( {
            ...
            onerror : '<font>请输入确认密码</font>'
        }).compareValidator( {
            desid : 'password',
            operateor : '=',
            onerror : '<font>两次密码不一致</font>'
        });
    },
     ...
     * 更新验证图片
     */
    refreshCaptchaImg : function(imageId, productCode) {
        $('#' + imageId)
                .attr("src", "/partner/ws/0.2/captcha/" + productCode + "?a=" + index.getRandomStr());
    },
    /**
     * 随机数字
     */
    ...
    modifyPassword : function() {
        ...
        $.ajax( {
            ...
            success : function(data) {
                ...
                $("#repeatpwWrap").show();
                $("#repeatpwTip").html("<font>修改密码失败。</font>");
            }
           }
        });
        return false;
   ...
   checkNewPasswordForm:function(){
        ...
        if (password == "" || password == null) {
            $('#hiddendotype').val("no");
            $("#repeatpwWrap").show();
            $("#repeatpwTip").html("<font>请输入密码</font>");
        }else if (/^[\u0391-\uFFE5\w]+$/.test(password) == false) {
            $('#hiddendotype').val("no");
            $("#repeatpwWrap").show();
            $("#repeatpwTip").html("<font>请输入中文字、英文字母、数字和下划线</font>");
        }else if(password.length<6) {
            $('#hiddendotype').val("no");
            $("#repeatpwWrap").show();
            $("#repeatpwTip").html("<font>密码太短了，最少为6位。</font>");
        }else{
            var len;
            var i;
            var isPassword = true;
            ...
            if (!isPassword || password.length > 16
                ...
                $('#hiddendotype').val("no");
                $("#repeatpwWrap").show();
                $("#repeatpwTip").html("<font>请输入6~16位长度的密码。</font>");
            }
            var temp1 = '';
            ...
            if (temp1 == password){
               $('#hiddendotype').val("no");
               $("#repeatpwWrap").show();
               $("#repeatpwTip").html("<font>密码不允许连续或重复</font>");
            }
            var a=0;
            var b=0;
            if (!isNaN(password)) {
        ...
        if (resPassword2 == "" || resPassword2 == null) {
            ...
            $('#hiddendotype').val("no");
            $("#repeatpwWrap").show();
            $("#repeatpwTip").html("<font>两次输入的密码不同!</font>");
    ...
    checkUserLoginOneFocus:function(id)
    {
        ...
        if(id=="account"){
          ...
          if(account_no.length > 255){
              $('#hiddendotype').val("no");
              $('#accounterror').html("<font>账号长度过长。</font>");
              $('#accounterror').attr("class","onError");
      ...
      //  }
            ...
            // if(result==0){
               ...
               if (account_no == "" || account_no == null) {
                   $('#hiddendotype').val("no");
                   $('#emailerror').html("<font>请输入登录邮箱或用户名</font>");
     ...
     /*          else if (/^([a-zA-Z0-9]+[_|\_|\.|\-]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/.test(account_no) == false) {
                   ...
                   $('#hiddendotype').val("no");
                   $('#emailerror').html("<font>请输入正确的邮箱地址</font>");
                   $('#emailerror').attr("class","onError");
               } */
               if(account_no.length > 255){
                   $('#hiddendotype').val("no");
                   $('#emailerror').html("<font>邮箱或用户名长度过长。</font>");
                   $('#emailerror').attr("class","onError");
          ...
          if (password == "" || password == null) {
            ...
            if (abbrev == "" || abbrev == null) {
                ...
                $('#hiddendotype').val("no");
                $('#abbreverror').html("<font>请输入单位缩写</font>");
                $('#abbreverror').attr("class","onError");
            }
        }
        if(flag)
            {
    ...
    initRegGo : function() {
        $
                .ajax( {
                    ...
                    success : function(data) {
                        if (data.isSuccess) {
                            var html = '<span class="font14px darkRedFont">注册成功！您的车友号是：' + data.userId + ' </span>可用来登录，请记住该车友号！';
    ...
    initRegGoValidator : function() {
        ...
        $('#nick_name').formValidator( {
            tipid : 'nick_name_tip',
            onshow : '&nbsp;',
            onfocus : '<font>请输入2～32个字符昵称（字母、数字、中文、特殊符号）</font>',
            oncorrect : '&nbsp;'
        }).functionValidator( {
            ...
            onerror : '<font>请输入2～32个字符的昵称（字母、数字、特殊符号）</font>'
        }).ajaxValidator( {
            type : 'get',
            ...
            onerror : '<font>此昵称太受欢迎，已经被人抢了。</font>',
            onwait : '<font>正在等待服务器验证结果...</font>'
        });
        $('#city').formValidator( {
            tipid : 'citys_tip',
            onshow : '&nbsp;',
            onfocus : '<font>请选择城市</font>',
            oncorrect : '&nbsp;'
        }).functionValidator( {
            fun : function(city) {
                if (!city)
        ...
        $("#birthday").focus(function() {
            ...
            onerror : '<font>请选择生日</font>'
        });
        $('#email')
                .formValidator( {
                    tipid : 'email_tip',
                    onshow : '&nbsp;',
                    onfocus : '<font>请输入邮箱</font>',
                    oncorrect : '&nbsp;'
                })
                .functionValidator( {
                    fun : function(email) {
