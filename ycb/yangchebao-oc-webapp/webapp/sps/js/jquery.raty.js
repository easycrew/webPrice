...
(function($) {
    ...
    $.fn.raty = function(settings) {
        ...
        var opt = $.extend({}, $.fn.raty.defaults, settings), $this = $(this), id = this.attr('id'), start = 0, starFile = opt.starOn, hint = '', target = opt.target, width = (opt.width) ? opt.width
                : (opt.number * opt.size + opt.number * 4);
        ...
        for ( var i = 1; i <= opt.number; i++) {
            ...
            $this.append(
                    '<img id="' + id + '-' + i + '" src="' + opt.path + starFile + '" alt="' + i
                            + '" title="' + hint + '" class="' + id + '"/>').append(
                    (i < opt.number) ? '&nbsp;' : '');
        }
        ...
        if (!opt.readOnly) {
            ...
            if (opt.cancel) {
                var stars = $('img.' + id), cancel = '<img src="' + opt.path + opt.cancelOff
                        + '" alt="x" title="' + opt.cancelHint + '" class="button-cancel"/>';
    ...
    function bindAll(context, opt, target) {
        var id = context.attr('id'), score = $('input#' + id + '-score'), stars = context.children('img.'
                + id);
        ...
        stars.bind(((opt.half) ? 'mousemove' : 'mouseover'), function(e) {
            ...
            if (opt.click) {
                opt.click.apply(context, [ score.val(), evt, opt ]);
            }
        });
    }
    ;
    ...
    function fillStar(id, score, opt) {
        var qtyStar = $('img.' + id).length, item = 0, range = 0, star, starOn;
    ...
    function fixHint(context, score, opt) {
        ...
        if (score != 0) {
            ...
            hint = (score > 0 && opt.number <= opt.hintList.length && opt.hintList[score - 1] !== null) ? opt.hintList[score - 1]
                    : score;
        } else {
            hint = opt.noRatedMsg;
        }
    ...
    function splitStar(context, score, opt) {
        var id = context.attr('id'), rounded = Math.ceil(score), diff = (rounded - score).toFixed(1);
    ...
    $.fn.raty.click = function(score, idOrClass) {
        var context = getContext(score, idOrClass, 'click'), options = $(idOrClass).data('options');
    ...
    $.fn.raty.readOnly = function(boo, idOrClass) {
        var context = getContext(boo, idOrClass, 'readOnly'), options = $(idOrClass).data('options'), cancel = context
                .children('img.button-cancel');
    ...
    $.fn.raty.start = function(score, idOrClass) {
        var context = getContext(score, idOrClass, 'start'), options = $(idOrClass).data('options');
