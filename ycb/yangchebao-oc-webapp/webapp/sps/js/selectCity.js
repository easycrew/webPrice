...
(function() {
	...
	(function() {
		 ...
		 * @param {Function}
		 *            listener 需要添加的监听器
		 * @remark 1. 不支持跨浏览器的鼠标滚轮事件监听器添加<br>
	...
	/**
	 * 常量，国家级别区域的类型代号， 用于cityclick派发的事件中，返回的citytype字段的识别
	 * 
	 * @static
	 */
	...
	/**
	 * 常量，省级别区域的类型代号， 用于cityclick派发的事件中，返回的citytype字段的识别
	 * 
	 * @static
	 */
	...
	/**
	 * 常量，城市级别区域的类型代号， 用于cityclick派发的事件中，返回的citytype字段的识别
	 * 
	 * @static
	 */
	 ...
	 * CityList类的构造函数
	 * 
	 * @class 城市列表类， 实例化该类后，可以帮助用户直接生成城市列表， 也可以通过接口获取城市数据。
	 * 
	 * @constructor
	 * @param {Json
	 *            Object} opts 可选的输入参数，非必填项。可输入选项包括：<br /> {"<b>container</b>" :
	 *            {String|HTMLElement}
	 *            需要提供界面方式展现的容器。如果此参数为空，则不提供界面方式，也没有cityclick的事件派发 <br />"<b>map</b>" :
	 *            {BMap} 实例化的map对象，如果传入此参数，则用户点击界面中的城市时，可以直接帮助用户定位到地图的相关城市位置}
	 * 
	 * @example
	 * <b>参考示例：</b><br />
	...
	BMapLib.CityList = function(opts) {
		...
		opts = opts || {};
		/**
		 * _opts是默认参数赋值。 下面通过用户输入的opts，对默认参数赋值
		 * 
		 * @private
		 * @type {Json}
		 */
	...
	CityList.prototype._getDataFromMapServer = function() {
		var me = this;
		scriptRequest(
				"http://api.map.baidu.com/library/CityList/1.2/src/data/CityData.js",
				function() {
					me._callback();
				});
	};
	...
	CityList.prototype._renderHtml = function() {
		...
		// 生成页面模式的界面
		var htm = [], data = this._data, vds = "javascript:void(0)", newData = {};

		htm.push("<div class='bmaplib_cityList' id='bmaplib_cityList_"
				+ this.guid + "'><dl>");
		...
		if (data.municipalities && data.municipalities.length > 0) {
			...
			for (var i = 0, n = data.municipalities.length; i < n; i++) {
				...
				htm.push("<a href='#vd#' code='" + _citylistCount + "'>" + mu.n
						+ "</a>");
			...
			htm.push("<a href='#vd#' code='9999' class='getTerminalCount'>全国</a>")
			htm.push("</dd>");
		}
		...
		for (var i = 0, n = data.provinces.length; i < n; i++) {
			...
			htm.push("<dt><a href='#vd#' class='black' code='" + _citylistCount
					+ "'>" + pv.n + "</a>：</dt><dd>");
			newData[_citylistCount] = {
				"g" : pv.g,
			...
			for (var j = 0, m = pv.cities.length; j < m; j++) {
				htm.push("<a href='#vd#' code='" + _citylistCount + "'>"
						+ pv.cities[j].n + "</a>");
				newData[_citylistCount] = {
					"g" : pv.cities[j].g,
					"t" : BMapLib.CITY_TYPE_CODE,
		...
		htm.push("</dl></div>");
		baidu.g(this._opts.container).innerHTML = htm.join("").replace(
				/#vd#/ig, vds);
		this._data = newData;
	};
	...
	CityList.prototype._bind = function() {
		...
		var tags = cl.getElementsByTagName("a"), me = this;
		for (var i = 0, n = tags.length; i < n; i++) {
			...
			(function() {
				...
				baidu.on(tg, "click", function() {
					 ...
					 * @event
					 * @param {Event
					 *            Object} e 回调函数会返回event参数，包括以下返回值： <br />{"<b>name</b> :
					 *            {String} 点击的区域名称, <br />"<b>center</b>：{BMap.Point}
					 *            点击区域合适显示的中心点位置, <br />"<b>zoom</b>：{Number}
					 *            点击区域合适显示的地图层级, <br />"<b>citytype</b>：{Number}
					 *            该区域的类型(全国0、省1、城市2)，区域的类型代号可见BMaplib下的<a
					 *            href="BMapLib.html#.COUNTRY_TYPE_CODE">COUNTRY_TYPE_CODE</a>、<a
					 *            href="BMapLib.html#.PROVINCE_TYPE_CODE">PROVINCE_TYPE_CODE</a>、<a
					 *            href="BMapLib.html#.CITY_TYPE_CODE">CITY_TYPE_CODE</a>三个常量。}
					 * 
					 * @example
					 * <b>参考示例：</b><br />
					 * myCityListObject.addEventListener("cityclick",
					 * function(e) { alert(e.name); });
					 */

					// 生成名为oncityclick的baidu.lang.Event对象
					// 并给该event对象添加上name、center、zoom和citytype等属性字段
					// 然后在此刻，将绑定在oncityclick上事件，全部赋予event参数，然后派发出去
					var event = new baidu.lang.Event("oncityclick"), code = baidu
							.getAttr(tg, "code"), json = me._data[code];
					if (!json) {
						return;
					}
	...
	CityList.prototype._execCss = function() {
		...
		if (!BMapLib.CityList._isStyleRender) {
			...
			if (baidu.g("_bmaplib_citylist_css")) {
				baidu.g("_bmaplib_citylist_css").parentNode.removeChild(baidu
						.g("_bmaplib_citylist_css"));
			}
		...
		if (_dt.provinces && _dt.provinces.length > 0) {
			...
			for (var i = 0, n = _dt.provinces.length; i < n; i++) {
				...
				for (var j = 0, m = _dt.provinces[i].cities.length; j < m; j++) {
					_newDt.provinces[i].cities
							.push(formatJson(_dt.provinces[i].cities[j]));
				}
			}
		}
	...
	/**
	 * 整理后端来的{"n":"北京","g":"116.395645,39.929986|12"}这种数据格式， 让它成为{"name" : "",
	 * "zoom" : 12, "center" : BMap.Point}这个可识别数据格式
	 * 
	 * @ignore
	 * @param {Json}
	 *            cityinfo 如{"n":"北京","g":"116.395645,39.929986|12"}的数据
	 * 
	...
	function formatJson(cityinfo) {
		...
		var dtsPts = dts[0].split(",");
		newInfo.center = {lat:dtsPts[0], lng:dtsPts[1]};
	...
	/*
	 * 根据地图当前大小计算适合的level，输入的level为后端根据固定大小确定的层级 @ignore @param {Number}
	 * 后端提供的level @param {Map} map实例
	 * 
