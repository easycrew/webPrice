...
$(document).ready(function() {
    ...
    $("#query").click(function() {
        ...
        if (departmentName != "" || $("#pDepartmentList").val() != '-1') {
            initPagination = true;
            queryDepartment(0, 10, departmentName, $("#pDepartmentList").val());
        } else {
            showErrorDialog("info", "查询参数不能为空");
        }
    });
...
function queryPageControlCallBack(page_index, container) {
    if (!initPagination) {
        queryDepartment(page_index, 10, lastDepartmentName, lastParentDepartmentId);
    }
}

var lastDepartmentName;
var lastParentDepartmentId;

function queryDepartment(pageindex, pagesize, departmentName, pDepartmentId) {
    lastDepartmentName = departmentName;
    lastParentDepartmentId = pDepartmentId;
    var queryParam = "";
    if (departmentName != "") {
        queryParam += "name=" + encodeURIComponent(departmentName) + "&";
    }

    if (pDepartmentId != '-1') {
        queryParam += "pId=" + pDepartmentId + "&";
    }

    var url = "/euc/ws/0.1/department/query?" + queryParam + "pageindex=" + pageindex + "&pagesize=" + pagesize;
    var success = function(data) {
        if (initPagination) {
            $("#paging_panel").pagination(data.totalPages * pagesize, {
                num_edge_entries : 1,
