...
function smallMessageShow(obj, level, message) {
 ...
 function loading(obj,message){
            var createLoadingHtml = function(obj){
                var loadingHtml = '<div class="wait_loading"><div class="loading_png"></div><div class="loading_message"></div></div>';
                $('<div id="alertBackground" class="overcurtainDiv"></div>').appendTo(obj);
                return $(loadingHtml).appendTo(obj);
            };
            var obj = $('#'+obj);
            obj.css('position','relative');
            var loading = $('.wait_loading',obj);
            if(loading.length == 0){
                loading = createLoadingHtml(obj);
            }
            loading.find('.loading_message').html(message);
            loading.jRadCenter(obj).css('zIndex',200);
            var docWidth = $(obj).width();
            var docHeight = $(obj).height();
            $('#alertBackground').show().css('opacity', 0.3).css({"height":docHeight,"width":docWidth});
  }
function unloading(obj){
            var loading = $('#'+obj).find('.wait_loading');
            $(loading).fadeOut(200).remove();
            $('#'+obj).find('#alertBackground').remove();
      }
